...
public final class DealDamageHandler {
	private enum AttackType {
		MELEE, RANGED, MAGIC, SUMMON, CHARGE
	}
	...
	private static int getMaxBaseDamage(GameCharacter p, int additionalWatk, PlayerSkillEffectsData attackSkill) {
		...
		int damage = (int) ((type.getMaxDamageMultiplier() * mainStat + secondaryStat) * (p.getWatk() + additionalWatk)
				/ 100);
		if (attackSkill != null)
	...
	public static void handleMeleeAttack(LittleEndianReader packet, GameClient gc) {
		...
		if (p.getInventory(InventoryType.EQUIPPED).get((short) -11) == null) {
			CheatTracker.get(gc).suspicious(CheatTracker.Infraction.POSSIBLE_PACKET_EDITING,
					"Tried to attack without a weapon");
			return;
		}
		AttackInfo attack = parseDamage(packet, AttackType.MELEE, p);
		p.getMap().sendToAll(writeMeleeAttack(p.getId(), attack, getMasteryLevel(p, AttackType.MELEE, attack.skill)),
				p);
	...
	public static void handleRangedAttack(LittleEndianReader packet, GameClient gc) {
		...
		boolean soulArrow = (attack.weaponClass == WeaponClass.BOW || attack.weaponClass == WeaponClass.CROSSBOW)
				&& p.isEffectActive(PlayerStatusEffect.SOUL_ARROW);
		if (!soulArrow) {
			boolean shadowStars = attack.weaponClass == WeaponClass.CLAW
					&& p.isEffectActive(PlayerStatusEffect.SHADOW_STARS);
			if (!shadowStars) { // consume ammo if shadow claw is not active
				...
				if (slot == null || slot.getQuantity() < useQty) {
					CheatTracker.get(gc).suspicious(CheatTracker.Infraction.POSSIBLE_PACKET_EDITING,
							"Tried to use nonexistent ranged weapon ammunition");
					return;
				}
				...
				if (slot.getQuantity() == 0 && !InventoryTools.isRechargeable(attack.ammoItemId)) {
					...
					gc.getSession().send(
							CommonPackets.writeInventoryUpdateSlotQuantity(InventoryType.USE, attack.ammoSlot, slot));
				}
			}
			if (attack.cashAmmoSlot != 0) { // NX throwing stars
				...
				if (slot == null) {
					CheatTracker.get(gc).suspicious(CheatTracker.Infraction.POSSIBLE_PACKET_EDITING,
							"Tried to use nonexistent cash shop stars");
					return;
				}
				attack.ammoItemId = slot.getDataId();
			}
		...
		p.getMap().sendToAll(writeRangedAttack(p.getId(), attack, getMasteryLevel(p, AttackType.RANGED, attack.skill)),
				p);
	...
	public static void handleMagicAttack(LittleEndianReader packet, GameClient gc) {
		...
		p.getMap().sendToAll(writeMagicAttack(p.getId(), attack, getMasteryLevel(p, AttackType.MAGIC, attack.skill)),
				p);
		...
		if (e != null) {
			...
			case Skills.POISON_MIST:
				...
				ScheduledFuture<?> poisonSchedule = Scheduler.getInstance().runRepeatedly(new Runnable() {
					@Override
					public void run() {
						for (MapEntity mo : p.getMap().getMapEntitiesInRect(mist.getBox(),
								EnumSet.of(EntityType.MONSTER)))
	...
	public static void handleEnergyChargeAttack(LittleEndianReader packet, GameClient gc) {
		...
		if (!p.isEffectActive(PlayerStatusEffect.ENERGY_CHARGE)) {
			CheatTracker.get(gc).suspicious(CheatTracker.Infraction.POSSIBLE_PACKET_EDITING,
					"Tried to power guard without having energy charge cast");
			return;
		}
		AttackInfo attack = parseDamage(packet, AttackType.CHARGE, p);
		p.getMap().sendToAll(
				writeEnergyChargeAttack(p.getId(), attack, getMasteryLevel(p, AttackType.CHARGE, attack.skill)), p);
		CheatTracker.get(gc).logTime("hpr", System.currentTimeMillis() + 5000);
	...
	public static void handlePreparedSkill(LittleEndianReader packet, GameClient gc) {
		...
		if (SkillDataLoader.getInstance().getSkill(skillId).isPrepared()) {
			...
			CheatTracker.get(gc).suspicious(CheatTracker.Infraction.CERTAIN_PACKET_EDITING,
	...
	private static void doPickPocketDrops(GameCharacter p, Mob monster, Entry<Integer, int[]> oned) {
		int delay = 0;
		int maxmeso = SkillDataLoader.getInstance().getSkill(Skills.PICK_POCKET)
				.getLevel(p.getEffectValue(PlayerStatusEffect.PICKPOCKET).getLevelWhenCast()).getX();
		...
		for (int eachd : oned.getValue()) {
			if (SkillDataLoader.getInstance().getSkill(Skills.PICK_POCKET).getLevel(p.getSkillLevel(Skills.PICK_POCKET))
					.makeChanceResult()) {
	...
	private static void giveMonsterDiseasesFromPassiveSkills(final GameCharacter player, Mob monster, WeaponClass weaponClass, int attackCount, AttackType type) {
		...
		if (type == AttackType.RANGED && weaponClass == WeaponClass.CLAW
				&& (level = player.getSkillLevel(Skills.VENOMOUS_STAR)) > 0) {
			e = SkillDataLoader.getInstance().getSkill(Skills.VENOMOUS_STAR).getLevel(level);
			for (int i = 0; i < attackCount; i++)
				MonsterStatusEffectTools.applyEffectsAndShowVisuals(monster, player, e);
		}
		if (type == AttackType.MELEE && weaponClass == WeaponClass.ONE_HANDED_MELEE
				&& (level = player.getSkillLevel(Skills.VENOMOUS_STAB)) > 0) {
			e = SkillDataLoader.getInstance().getSkill(Skills.VENOMOUS_STAB).getLevel(level);
			for (int i = 0; i < attackCount; i++)
				MonsterStatusEffectTools.applyEffectsAndShowVisuals(monster, player, e);
		}

		// MP Eater - just stack them until the monster has no MP if we leveled more
		// than one of them!
		if ((level = player.getSkillLevel(Skills.FP_MP_EATER)) > 0) {
			...
			if (e.makeChanceResult()) {
				...
				if (absorbMp > 0) {
					...
					player.getClient().getSession().send(GamePackets.writeSelfVisualEffect(
							StatusEffectTools.ACTIVE_BUFF, Skills.FP_MP_EATER, level, (byte) -1));
					player.getMap().sendToAll(GamePackets.writeBuffMapVisualEffect(player,
							StatusEffectTools.ACTIVE_BUFF, Skills.FP_MP_EATER, level, (byte) -1), player);
				}
			}
		}
		if ((level = player.getSkillLevel(Skills.IL_MP_EATER)) > 0) {
			...
			if (e.makeChanceResult()) {
				...
				if (absorbMp > 0) {
					...
					player.gainMp(absorbMp);
					player.getClient().getSession().send(GamePackets.writeSelfVisualEffect(
							StatusEffectTools.ACTIVE_BUFF, Skills.IL_MP_EATER, level, (byte) -1));
					player.getMap().sendToAll(GamePackets.writeBuffMapVisualEffect(player,
							StatusEffectTools.ACTIVE_BUFF, Skills.IL_MP_EATER, level, (byte) -1), player);
				}
			}
		}
		if ((level = player.getSkillLevel(Skills.CLERIC_MP_EATER)) > 0) {
			...
			if (e.makeChanceResult()) {
				...
				if (absorbMp > 0) {
					...
					player.gainMp(absorbMp);
					player.getClient().getSession().send(GamePackets.writeSelfVisualEffect(
							StatusEffectTools.ACTIVE_BUFF, Skills.CLERIC_MP_EATER, level, (byte) -1));
					player.getMap().sendToAll(GamePackets.writeBuffMapVisualEffect(player,
							StatusEffectTools.ACTIVE_BUFF, Skills.CLERIC_MP_EATER, level, (byte) -1), player);
				}
			}
		}
		...
		if ((level = player.getSkillLevel(Skills.ENERGY_CHARGE)) > 0) {
			if (player.getEnergyCharge() != 10000) {
				...
				if (player.getEnergyCharge() == 10000) {
					player.addToActiveEffects(PlayerStatusEffect.ENERGY_CHARGE,
							new PlayerStatusEffectValues(e, (short) 10000));
					...
					player.addCancelEffectTask(e, Scheduler.getInstance().runAfterDelay(new Runnable() {
						...
						public void run() {
							...
							Map<PlayerStatusEffect, Short> updatedStats = Collections
									.singletonMap(PlayerStatusEffect.ENERGY_CHARGE, Short.valueOf((short) 0));
							player.getClient().getSession()
									.send(GamePackets.writeUsePirateSkill(updatedStats, 0, 0, (short) 0));
							player.getMap().sendToAll(GamePackets.writeBuffMapPirateEffect(player, updatedStats, 0, 0),
									player);
						}
					}, e.getDuration()), level, System.currentTimeMillis() + e.getDuration());
				}
				Map<PlayerStatusEffect, Short> updatedStats = Collections.singletonMap(PlayerStatusEffect.ENERGY_CHARGE,
						Short.valueOf(player.getEnergyCharge()));
				...
				player.getClient().getSession().send(GamePackets.writeSelfVisualEffect(StatusEffectTools.PASSIVE_BUFF,
						Skills.ENERGY_CHARGE, level, (byte) -1));
				player.getMap().sendToAll(GamePackets.writeBuffMapVisualEffect(player, StatusEffectTools.PASSIVE_BUFF,
						Skills.ENERGY_CHARGE, level, (byte) -1), player);
			}
		}
	}
	...
	private static void applyAttack(AttackInfo attack, AttackType type, final GameCharacter player) {
		...
		if (attackEffect != null && attack.skill != 0) { // attack skills
			...
			case Skills.CHARGED_BLOW: {
				...
				if (chargeBuff == null) {
					CheatTracker.get(player.getClient()).suspicious(CheatTracker.Infraction.POSSIBLE_PACKET_EDITING,
							"Tried to use Charged Blow without Charge buff");
					return;
				}
				byte advancedBlowLvl = player.getSkillLevel(Skills.ADVANCED_CHARGE_BLOW);
				if (advancedBlowLvl == 0 || Rng.getGenerator().nextInt(100) >= SkillDataLoader.getInstance()
						.getSkill(Skills.ADVANCED_CHARGE_BLOW).getLevel(advancedBlowLvl).getX())
			...
			case Skills.AXE_COMA:
				if (combo == null) {
					CheatTracker.get(player.getClient()).suspicious(CheatTracker.Infraction.POSSIBLE_PACKET_EDITING,
							"Tried to use Panic or Coma without Combo Attack buff");
					return;
				}
		...
		if (combo != null && !attack.allDamage.isEmpty()) { // TODO: make sure to increment counter for final attack as
															// well
		...
		for (Entry<Integer, int[]> oned : attack.allDamage.entrySet()) {
			...
			if (monster != null) {
				...
				case Skills.ENERGY_DRAIN:
					int addHp = (int) ((double) totDamageToOneMonster * (double) SkillDataLoader.getInstance()
							.getSkill(attack.skill).getLevel(player.getSkillLevel(attack.skill)).getX() / 100.0);
					addHp = Math.min(monster.getMaxHp(), Math.min(addHp, player.getCurrentMaxHp() / 2));
					player.gainHp(addHp);
					break;
				case Skills.HEAVENS_HAMMER:
					// TODO: min damage still needs to be calculated. Using -20% as mindamage in the
					// meantime seems to work
					// totDamageToOneMonster = (int)
					// (player.calculateMaxBaseDamage(player.getTotalWatk()) *
					// (SkillDataLoader.getInstance().getSkill(Skills.HEAVENS_HAMMER).getLevel(player.getSkillLevel(Skills.HEAVENS_HAMMER)).getDamage()
					// / 100));
					// totDamageToOneMonster = (int) (Math.floor(Rng.getGenerator().nextDouble() *
					// (totDamageToOneMonster * .2) + totDamageToOneMonster * .8));
					break;
				case Skills.SACRIFICE:
					player.setHp(
							(short) Math.max(player.getHp() - totDamageToOneMonster * attackEffect.getX() / 100, 1));
					break;
				case Skills.POISON_MIST:
					// TODO: could Poison Mist poison a monster as a basic attack?
					// if (totDamageToOneMonster > 0 && monster.isAlive() && attackEffect != null)
					// MonsterStatusEffectTools.applyEffectsAndShowVisuals(monster, player,
					// attackEffect);
					break;
	...
	private static byte getMasteryLevel(GameCharacter p, AttackType type, int skill) {
		...
		for (Integer skillId : skills) {
			PlayerSkillEffectsData skillData = SkillDataLoader.getInstance().getSkill(skillId.intValue())
					.getLevel(p.getSkillLevel(skillId.intValue()));
			if (skillData != null)
